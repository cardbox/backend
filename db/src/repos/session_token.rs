...
impl SessionTokenRepo for Database {
    ...
    async fn token_find(&self, token: String) -> RepoResult<Option<models::SessionToken>> {
        Ok(sqlx::query_as!(
            SessionToken,
            // language=PostgreSQL
            r#"
    ...
    async fn token_find_by_user(&self, user_id: Uuid) -> RepoResult<Option<models::SessionToken>> {
        Ok(sqlx::query_as!(
            SessionToken,
            // language=PostgreSQL
            r#"
    ...
    async fn token_create(&self, token: models::SessionToken) -> RepoResult<models::SessionToken> {
        Ok(sqlx::query_as!(
            SessionToken,
            // language=PostgreSQL
            r#"
